...
// Re-export.
pub use mock::http_request;
pub use mock::mock;
pub use mock::mock_with_delay;
pub use mock::times_called;
pub use request::create_request;
pub use response::create_response;
pub use transform::create_transform_context;
...
mod test {
    ...
    async fn test_http_request_sequentially() {
        // Arrange
        let request_a = create_request().url("a").build();
        let request_b = create_request().url("b").build();
        let request_c = create_request().url("c").build();
