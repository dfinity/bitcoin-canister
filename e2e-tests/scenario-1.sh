...
if [[ $GET_UTXOS_QUERY_REPLICATED_CALL != *"Composite query cannot be called in replicated mode"* ]]; then
  echo "FAIL"
  exit 1
fi
...
if [[ $GET_BALANCE_QUERY_REPLICATED_CALL != *"Composite query cannot be called in replicated mode"* ]]; then
  echo "FAIL"
  exit 1
fi
